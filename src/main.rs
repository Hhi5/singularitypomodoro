...
struct FileCfg {
    ...
    spin: Option<f32>,
    pomodoro_work_minutes: Option<f32>,
    pomodoro_break_minutes: Option<f32>,
}
...
fn parse_config(text: &str) -> FileCfg {
    ...
    for line in text.lines() {
        ...
        match k {
            ...
            "pomodoro_work_minutes" => cfg.pomodoro_work_minutes = v.parse().ok(),
            "pomodoro_break_minutes" => cfg.pomodoro_break_minutes = v.parse().ok(),
            _ => {}
        }
    }
    cfg
...
impl State {
    ...
    fn tick_pomodoro(&mut self, idle_secs: f32) {
        ...
        let dt = (now - self.last_pomodoro_tick).as_secs_f32().min(0.1);
        self.last_pomodoro_tick = now;
    ...
    fn tick_center(&mut self) {
        ...
        let target = match self.pinned_px {
            Some(p) => p,
            None => {
                self.drift_phase += dt * 0.12 * current_drift_speed;
                let l = lissa(self.drift_phase);
                [
                    self.roam_pos[0]
...
fn main() {
    ...
    if let Some(v) = startup_cfg.pomodoro_work_minutes {
        state.pomodoro_work_minutes = v;
    }
    if let Some(v) = startup_cfg.pomodoro_break_minutes {
        state.pomodoro_break_minutes = v;
    }
